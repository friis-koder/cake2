...
HTML;
        foreach ($coverageData as $file => $data) {
            $fileData = file($file);
            $output .= $this->generateDiff($file, $fileData, $data);
        }
     ...
     */
    public function generateDiff(string $filename, array $fileLines, array $coverageData): string
    {
        $output = '';
        $diff = array();
        ...
        foreach ($fileLines as $lineno => $line) {
            ...
            if (!empty($coverageData[$lineno]) && is_array($coverageData[$lineno])) {
                ...
                $class = 'uncovered';
                    ...
                    $lineno,
                    $coverageData
                ) && ($coverageData[$lineno] === -2 || $coverageData[$lineno] === null)) {
                $class .= ' dead';
            }
     ...
     */
    protected function _guessSubjectName(ReflectionClass $testReflection): string
    {
     ...
     * @return string
     */
    protected function _paintLine(string $line, int $linenumber, string $class, array $coveringTests): string
    {
        $coveredBy = '';
     ...
     * @return string
     */
    public function coverageScript(): string
    {
        return <<<HTML
...
HTML;
     ...
     * @return string
     */
    public function coverageHeader(string $filename, string $percent): string
    {
        $hash = md5($filename);
