...
class CakeHtmlReporter extends CakeBaseReporter
{
    ...
    public function testCaseList()
    {
        ...
        foreach ($testCases as $testCase) {
            ...
            $title = implode(' / ', $title);
            $buffer .= "<li><a href='" . $this->baseUrl() . "?case=" . urlencode(
                    $testCase
                ) . $urlExtra . "'>" . $title . "</a></li>\n";
        }
    ...
    protected function _paintLinks()
    {
        ...
        echo "<p><a href='" . $this->baseUrl() . $show . "'>Run more tests</a> | <a href='" . $this->baseUrl(
        ...
        echo "<a href='" . $this->baseUrl(
            ) . $query . "&amp;code_coverage=true&amp;show_passes=1&amp;debug=1'>All options enabled</a></p>\n";
    }
     ...
     */
    public function paintFail(AssertionFailedError $message, $test)
    {
        ob_start();
        ...
        if (strpos($className, "PHPUnit_") === false) {
            list($show, $query) = $this->_getQueryLink();
            echo "<div class='msg'><a href='" . $this->baseUrl() . $query . "&amp;filter=" . $test->getName(
                ) . "'>" . __d('cake_dev', 'Rerun only this test: %s', $testName) . "</a></div>\n";
        }
     ...
     * top level test.
     *
     * @param Test $test Test method that just passed
    ...
    public function paintPass(Test $test, $time = null)
    {
        ...
        if (isset($this->params['showPasses']) && $this->params['showPasses']) {
            ...
            echo "<br />" . $this->_htmlEntities($test->name) . " ($time seconds)\n";
            echo "</li>\n";
        }
     ...
     */
    public function paintException(Exception $exception, $test)
    {
        ob_start();
     ...
     *
     * @param Exception $message Text of skip condition.
     * @param TestCase $test the test method skipped
     */
    public function paintSkip(Exception $message, TestCase $test)
    {
        ob_start();
     ...
     */
    protected function _htmlEntities(string $message): string
    {
    ...
    /**
     * Returns the query string formatted for output in links
     *
     * @return array
     */
