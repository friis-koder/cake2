...
class TestHelper extends Helper
{
     ...
     * @return string
     */
    public function parseAttributes($options, ?array $exclude = null, string $insertBefore = ' ', ?string $insertAfter = null): string
    {
...
class HelperTest extends CakeTestCase
{
     ...
     */
    public function testSetEntity(string $entity, array $expected, string $modelKey, string $fieldKey)
    {
    ...
    public function testUrlConversion()
    {
        ...
        $result = $this->Helper->url(
            array('controller' => 'posts', 'action' => 'index', 'page' => '1" onclick="alert(\'XSS\');"')
        );
        ...
        $this->assertEquals(
            "/posts/index/param:%257Baround%2520here%257D%255Bthings%255D%255Bare%255D%2524%2524",
            $result
        );
    ...
    public function testAssetTimestamp()
    {
        ...
        $result = $this->Helper->assetTimestamp(Configure::read('App.cssBaseUrl') . 'cake.generic.css');
        $this->assertRegExp(
            '/' . preg_quote(Configure::read('App.cssBaseUrl') . 'cake.generic.css?', '/') . '[0-9]+/',
            $result
        );
