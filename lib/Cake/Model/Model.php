...
// @codingStandardsIgnoreEnd
        ...
        foreach ($this->_associationKeys[$type] as $key) {
            if (!isset($assoc[$key]) || $assoc[$key] === null) {
                ...
                switch ($key) {
                    ...
                    case 'foreignKey':
                        $data = (($type === 'belongsTo') ? Inflector::underscore($assocKey) : Inflector::singularize(
                                $this->table
                            )) . '_id';
                        break;
    ...
    public function setSource($tableName)
    {
        ...
        if (method_exists($db, 'listSources')) {
            ...
            if (is_array($sources) && !in_array(
                    strtolower($this->tablePrefix . $tableName),
                    array_map('strtolower', $sources)
                )) {
    ...
    public function deconstruct($field, $data)
    {
        ...
        if ($type === 'datetime' || $type === 'timestamp' || $type === 'date') {
            foreach ($dateFields as $key => $val) {
                ...
                if (!isset($data[$val]) || isset($data[$val]) && (empty($data[$val]) || substr(
                            $data[$val],
                            0,
                            1
                        ) === '-')) {
    ...
    public function saveMany($data = null, $options = array())
    {
        ...
        try {
            ...
            foreach ($data as $key => $record) {
                ...
                $validates = ($validates && ($saved === true || (is_array($saved) && !in_array(
                                false,
                                Hash::flatten($saved),
                                true
                            ))));
    ...
    public function saveAssociated($data = null, $options = array())
    {
        ...
        if ($options['validate'] === 'first') {
            ...
            if ((!$validates && $options['atomic']) || (!$options['atomic'] && in_array(
                        false,
                        Hash::flatten($validates),
                        true
                    ))) {
                return $validates;
            }
        ...
        try {
            ...
            foreach ($data as $association => $values) {
                ...
                if ($validates) {
                    ...
                    $validates = ($saved === true || (is_array($saved) && !in_array(
                                false,
                                Hash::flatten($saved),
                                true
                            )));
            ...
            if ($validates && !($this->create(null) !== null && $this->save(
                        $data,
                        array('atomic' => false) + $options
                    ))) {
    ...
    protected function _addToWhiteList($key, $options)
    {
        ...
        if (!empty($options['fieldList']) && is_array($options['fieldList']) && Hash::dimensions(
                $options['fieldList']
            ) < 2) {
    ...
    protected function _deleteDependent($id, $cascade)
    {
        ...
        foreach (array_merge($this->hasMany, $this->hasOne) as $assoc => $data) {
            ...
            if ($data['foreignKey'] === false && $data['conditions'] && in_array(
                    $this->name,
    ...
    protected function _findCount($state, $query, $results = array())
    {
        if ($state === 'before') {
            ...
            } elseif (method_exists($db, 'expression') && is_string($query['fields']) && !preg_match(
                    '/count/i',
                    $query['fields']
                )) {
                $query['fields'] = $db->calculate(
                    $this,
                    'count',
                    array(
