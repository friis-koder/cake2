...
class Helper extends CakeObject
{
     ...
     */
    public function url($url = null, $full = false): string
    {
     ...
     */
    public function assetUrl($path, $options = array()): string
    {
        if (is_array($path)) {
     ...
     */
    protected function _parseAttributes($options, $exclude = null, $insertBefore = ' ', $insertAfter = null): string
    {
     ...
     *
     * @param mixed $key The name of the attribute to create
     * @param string $value The value of the attribute to create.
     * @param bool $escape Define if the value must be escaped
     *
     * @return string The composed attribute.
     */
    protected function _formatAttribute($key, string $value, bool $escape = true): string
    {
        if (is_array($value)) {
            $value = implode(' ', $value);
        }
     ...
     */
    public function setEntity(string $entity, bool $setScope = false)
    {
        if ($entity === null) {
     ...
     */
    protected function _initInputField(string $field, array $options = array()): array
    {
        if ($field !== null) {
            $this->setEntity($field);
        }
     ...
     */
    public function addClass(array $options = array(), ?string $class = null, string $key = 'class'): array
    {
     ...
     */
    public function beforeRenderFile(string $viewFile): void
    {
    }
     ...
     */
    public function afterRenderFile(string $viewFile, string $content): void
    {
    }
     ...
     * @return array|null
     */
    protected function _selectedArray($data, string $key = 'id'): ?array
    {
        if (!is_array($data)) {
            $model = $data;
    ...
    protected function _clean(): void
    {
        ...
        $this->_cleaned = preg_replace(
            '#([a-z]*)[\x00-\x20]*=*([\'\"]*)[\x00-\x20]*-moz-binding[\x00-\x20]*:#iUu',
            '$1=$2nomozbinding...',
            $this->_cleaned
        );

        $this->_cleaned = preg_replace(
            '#([a-z]*)[\x00-\x20]*=([\'\"]*)[\x00-\x20]*data[\x00-\x20]*:#Uu',
            '$1=$2nodata...',
            $this->_cleaned
        );
        $this->_cleaned = preg_replace(
